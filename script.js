...
const translations = {
    en: {
        ...
        step1Label: "Select your save file (.dat or others)",
        ...
        footerDate: "Current Date: May 3, 2025", // You might want to make this dynamic
        /* Status Messages */
        statusSelectFile: "Please select a file first.",
        statusDecryptSuccess: "File decrypted successfully and loaded into the editor.",
        statusDecryptError: "Error decrypting or parsing the file. Is it a valid HCR2 file (v1.33 or earlier)? Details: {error}",
        statusFileReadError: "Error reading the file.",
        statusFileTooShort: "File is too short to be a valid file.",
        ...
        statusFileUnknownHeader: "Could not determine file type based on header.", // Added message
    },
    fr: {
        ...
        step1Title: "Charger le Fichier",
        step1Label: "Sélectionnez votre fichier (.dat ou autres)",
        ...
        footerDate: "Date Actuelle : 3 mai 2025", // You might want to make this dynamic
        /* Status Messages */
        ...
        statusDecryptError: "Erreur lors du déchiffrement ou de l'analyse du fichier. Est-ce un fichier HCR2 valide (v1.33 ou <) ? Détails : {error}",
        statusFileReadError: "Erreur de lecture du fichier.",
        statusFileTooShort: "Le fichier est trop court pour être valide.",
        ...
        statusFileUnknownHeader: "Impossible de déterminer le type de fichier basé sur l'en-tête.", // Added message
    }
};
...
const key = [
    ...
    0xA6, 0x0D, 0xD9, 0EB, 0x58, 0xFC, 0x39, 0x73, 0x80, 0x24, 0x36, 0xD4, 0xCC, 0xC3, 0x6C, 0x32,
...
/* Determine header size based on file bytes */
function determineHeaderSize(fileBytes) {
     if (!fileBytes || fileBytes.length < 4) {
         return 0; // Cannot determine header size if file is too short
     }
        ...
        fileBytes[2] === 0x58)
    {
        ...
        return 1; // Assume standard .dat header
    }
}
...
decryptButton.addEventListener('click', () => {
    ...
    reader.onload = (e) => {
        try {
            ...
            if (originalFileBytes.length < 1) { // Minimum size for any file
            ...
            if (headerSize === 0) {
                 showStatus('statusFileUnknownHeader', 'danger');
                 originalFileBytes = null;
                 return;
            }
...
encryptButton.addEventListener('click', () => {
    ...
    try {
        ...
        // Determine headerSize from original file bytes
        const headerSize = determineHeaderSize(originalFileBytes);
        ...
        // Concatenate original header with modified content for encryption
...
downloadButton.addEventListener('click', () => {
    ...
    try {
        ...
        a.href = url;
        // Use the original file name but append _mod.dat
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Using 'input' for live update as the user drags the color picker
    // Use 'change' if you prefer the color to update only when the user
    // selects a color and closes the picker, or if you add a validation button.
    bgColorPicker.addEventListener('input', (event) => {
        body.style.backgroundColor = event.target.value;
    });

    // Optional: Save the chosen color to localStorage so it persists across visits
    // This is typically triggered on the 'change' event
...
function rgbToHex(rgb) {
    ...
    // Handle transparent color explicitly if needed
     if (rgb === 'rgba(0, 0, 0, 0)') {
         return 'transparent'; // Or a specific hex for transparent if desired
     }
    ...
    if (!rgbMatch) {
         ...
         if (rgbaMatch && parseFloat(rgbaMatch[4]) === 1) {
             // If alpha is 1, treat as opaque RGB
             const toHex = (c) => {
        ...
        return null; // Not a standard rgb or opaque rgba format
    }
