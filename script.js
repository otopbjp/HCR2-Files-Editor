...
fileInput.addEventListener('change', (event) => {
    ...
    if (file) {
        ...
        fileNameDisplay.textContent = `Selected file: ${originalFileName}`; // Static text
        ...
        originalFileName = '';
        fileNameDisplay.textContent = 'No file selected.'; // Static text
...
decryptButton.addEventListener('click', () => {
    ...
    if (!file) {
        showStatus('Please select a file first.', 'warning'); // Static text
        return;
    }
    ...
    reader.onload = (e) => {
        try {
            ...
            if (headerSize === 0) {
                 showStatus('Error: Only FNX JSON files are supported.', 'danger'); // Static text
                 originalFileBytes = null;
                 editor.setValue('');
            ...
            if (originalFileBytes.length < headerSize) {
                 showStatus('Error: File is too short to be a valid FNX JSON file.', 'danger'); // Static text
                 originalFileBytes = null;
                 editor.setValue('');
                 editor.setOption("readOnly", true);
                 encryptButton.disabled = true;
            ...
            showStatus('File decrypted successfully and loaded into the editor.', 'success'); // Static text

        } catch (error) {
            console.error("Decryption or parsing error:", error);
            showStatus(`Error decrypting or parsing the file: ${error.message}`, 'danger'); // Static text with error detail
            editor.setValue('');
            editor.setOption("readOnly", true);
            encryptButton.disabled = true;
            downloadButton.disabled = true;
    ...
    reader.onerror = () => {
        showStatus('Error reading the file.', 'danger'); // Static text
        editor.setValue('');
        editor.setOption("readOnly", true);
        encryptButton.disabled = true;
        downloadButton.disabled = true;
...
encryptButton.addEventListener('click', () => {
    if (!editor || !originalFileBytes) {
        showStatus('Please decrypt a file first.', 'warning'); // Static text
        return;
    }
    ...
    try {
        ...
        const headerSize = determineHeaderSize(originalFileBytes);
         ...
         if (headerSize === 0) {
             // This case should ideally not happen if decrypt was successful, but included for safety
              showStatus('Error: Could not determine file type for encryption.', 'danger'); // Static text
              downloadButton.disabled = true;
              encryptedModifiedBytes = null;
              return;
          }
        ...
        showStatus('Changes encrypted successfully. Ready to download.', 'success'); // Static text

    } catch (error) {
        console.error("JSON validation or encryption error:", error);
        showStatus(`Error during JSON validation or encryption: ${error.message}`, 'danger'); // Static text with error detail
        downloadButton.disabled = true;
        encryptedModifiedBytes = null;
    }
});
...
downloadButton.addEventListener('click', () => {
    if (!encryptedModifiedBytes) {
        showStatus('Please encrypt the changes first.', 'warning'); // Static text
        return;
    }
    ...
    try {
        ...
        showStatus('Download initiated.', 'info'); // Static text

    } catch (error) {
        console.error("Download error:", error);
        showStatus(`An error occurred while preparing the download: ${error.message}`, 'danger'); // Static text with error detail
    }
});
